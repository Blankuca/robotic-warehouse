...
for size, diff, agents in _perms:
    # normal tasks
    gym.register(
        id=f"newrware-{size}-{agents}ag{diff}-v1",
        entry_point="newrware.warehouse:Warehouse",
        kwargs={
            "column_height": 8,
...
def image_registration():
    ...
    for size, diff, obs_type, directional, agents in _perms:
        ...
        if directional != "" and obs_type == "":
            ...
            continue
        gym.register(
            id=f"newrware{obs_type}{directional}-{size}-{agents}ag{diff}-v1",
            entry_point="newrware.warehouse:Warehouse",
            kwargs={
                "column_height": 8,
                "shelf_rows": _sizes[size][0],
...
def full_registration():
    ...
    for size, diff, obs_type, sensor_range, directional, agents, column_height in _perms:
        ...
        if directional != "" and obs_type == "":
            ...
            continue
        gym.register(
            id=f"newrware{obs_type}{directional}{sensor_range}-{size}-{column_height}h-{agents}ag{diff}-v1",
            entry_point="newrware.warehouse:Warehouse",
            kwargs={
    ...
    for rows, cols, column_height, agents, req, rew, obs_type, sensor_range, directional in _perms:
        gym.register(
            id=f"newrware{obs_type}{directional}{sensor_range}-{rows}x{cols}-{column_height}h-{agents}ag-{req}req-{rew}-v1",
            entry_point="newrware.warehouse:Warehouse",
            kwargs={
                "column_height": column_height,
                "shelf_rows": rows,
